...
class LegacyMarketDataProviderTest(unittest.TestCase):
    ...
    def test_exchange_normalization_matches_current_market_module(self):
        ...
        binance = LegacyMarketDataProvider(
            "binance",
            ticker_function=ticker,
            klines_function=klines,
        )
        ...
        self.assertEqual(binance.exchange, "binance")
        self.assertEqual(
            ticker.call_args_list,
            [
                call("ETHUSDT", exchange="okx"),
                call("ETHUSDT", exchange="binance"),
            ],
        )


class RootMarketCompatibilityTest(unittest.TestCase):
    @patch("market.BinanceMarketDataProvider")
    ...
    def test_root_functions_dispatch_without_changing_call_shape(
        ...
        okx_provider_class,
        binance_provider_class,
    ):
        binance_provider_class.return_value.get_ticker.return_value = {
            "price": 100.0
        }
        ...
        bybit_provider_class.assert_not_called()
        binance_provider_class.assert_called_once_with()
        binance_provider_class.return_value.get_ticker.assert_called_once_with(
            "BTCUSDT"
        )
    ...
    def test_private_normalizers_remain_compatible(self):
        ...
        self.assertEqual(market._normalize_exchange("BINANCE"), "binance")
